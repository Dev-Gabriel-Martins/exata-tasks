              ...
              <tbody class="divide-y divide-gray-200">
                <tr v-for="task in $page.props.tasks.data" :key="task.id">
                  ...
                  <td class="px-6 py-4">
                    ...
                    <span v-else-if="task.status === 1" class="bg-blue-100 text-blue-800 px-2.5 py-0.5 rounded">Em
                      Andamento</span>
                    <span v-else class="bg-green-100 text-green-800 px-2.5 py-0.5 rounded">Concluída</span>
                  </td>
                  <td class="px-6 py-4 text-right">
                    <PrimaryButton @click="editTask(task)">Editar</PrimaryButton>
                    <DangerButton @click="deleteTask(task.id)">Apagar</DangerButton>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg p-6">
          ...
          <form @submit.prevent="submitForm">
            ...
            <div class="flex justify-end gap-4">
              ...
              <DangerButton v-if="isEditMode" @click="resetForm">Cancelar</DangerButton>
            </div>
          </form>
        </div>
      </div>
